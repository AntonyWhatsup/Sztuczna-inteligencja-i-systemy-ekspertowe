import os, csv
from panel import (Panel, best_orientation, fill_roof_with_panels, 
...
# ІМПОРТ ЗМІНЕНО: тепер імпортуємо функцію сітки
...
if __name__ == "__main__":
    ...
    # --- ОНОВЛЕНИЙ "ГЕНЕРАЦІЙНИЙ" ПІДХІД ---
    
    # 1. Спочатку визначаємо найкращу ОРІЄНТАЦІЮ (портрет чи альбом)
    ...
    # 2. Перевіряємо 9 варіантів вирівнювання та зберігаємо топ-3
    print("[Optimizing] Trying 9 layout alignment 'generations'...")
    align_x_opts = ["left", "center", "right"]
    align_y_opts = ["top", "center", "bottom"] # 'top' = біля 'kalenicy'

    # Зберігаємо (N_total, align_x, align_y, data_L, data_R)
    best_variants: List[Tuple[int, str, str, dict, dict]] = []
    
    for align_x in align_x_opts:
        for align_y in align_y_opts:
            # А. Створюємо базову сітку з цим вирівнюванням
            base_L = fill_roof_with_panels(roof_left,  panel_base, BORDER, BORDER, 
                                           orientation=orientation, 
                                           align_x=align_x, align_y=align_y)
            base_R = fill_roof_with_panels(roof_right, panel_base, BORDER, BORDER, 
                                           orientation=orientation,
                                           align_x=align_x, align_y=align_y)
                                           
            # Б. Видаляємо панелі, що стикаються з перешкодами
            data_L_variant = fill_with_obstacles(roof_left,  panel_base, base_L, obstacles_left)
            data_R_variant = fill_with_obstacles(roof_right, panel_base, base_R, obstacles_right)
            
            # В. Заповнюємо всі прогалини портретними панелями
            data_L_variant = augment_with_gap_portraits(roof_left,  panel_base, data_L_variant, obstacles_left)
            data_R_variant = augment_with_gap_portraits(roof_right, panel_base, data_R_variant, obstacles_right)
            
            # Г. Рахуємо результат та зберігаємо
            N_total = data_L_variant["total_panels"] + data_R_variant["total_panels"]
            ...
            # Оновлюємо топ-3 (сортуємо і залишаємо лише перші 3)
            best_variants.sort(key=lambda x: x[0], reverse=True)
            best_variants = best_variants[:3]
            

    # Обираємо найкращі дані з 9 "генерацій" (це буде перший елемент у best_variants)
    ...
    # --- КІНЕЦЬ НОВОГО ПІДХОДУ ---

    # валидация (перевіряємо найкращий варіант)
    ...
    # визуализация → PNG. Викликаємо нову функцію для порівняння
    png_path = os.path.join(cfg.out_dir, "top_view_comparison.png") if cfg.save_png else None
    ...
    # НОВИЙ ВИКЛИК: малюємо топ-3 варіанти на одному графіку
    draw_comparison_grid(roof_left, roof_right, panel_for_plot, obstacles_left, obstacles_right, 
                           best_variants, save_path=png_path, show=True)

    # CSV координаты (експортуємо тільки найкращий варіант)
    if cfg.save_csv:
